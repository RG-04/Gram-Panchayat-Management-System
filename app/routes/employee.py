...
def certificate_file(aadhaar, category, name):
    ...
    if not result or not result[0][0]:
        ...
        return redirect(url_for('employee.view_certificate', 
                               aadhaar=aadhaar, 
                               category=category, 
                               name=name))
...
@employee_bp.route('/citizen/<aadhaar>/certificate/<category>/<name>/update', methods=['POST'])
@role_required(['employee'])
...
def upload_certificate(aadhaar):
    ...
    if request.method == 'POST':
        ...
        if count > 0:
            flash('A certificate with this category and name already exists for this citizen', 'error')
            return render_template(
                'employee/upload_certificate.html',
                citizen_id=aadhaar,
                citizen_name=citizen_name[0][0],
